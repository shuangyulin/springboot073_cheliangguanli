<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="准驾车型" prop="zhunjiachexing">
          <el-input v-model="ruleForm.zhunjiachexing" 
              placeholder="准驾车型" clearable  :readonly="ro.zhunjiachexing"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="准驾车型" prop="zhunjiachexing">
              <el-input v-model="ruleForm.zhunjiachexing" 
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.jiashizhengtu" label="驾驶证图" prop="jiashizhengtu">
          <file-upload
          tip="点击上传驾驶证图"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.jiashizhengtu?ruleForm.jiashizhengtu:''"
          @change="jiashizhengtuUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.jiashizhengtu" label="驾驶证图" prop="jiashizhengtu">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.jiashizhengtu.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="有效日期" prop="youxiaoriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.youxiaoriqi" 
                type="date"
                placeholder="有效日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.youxiaoriqi" label="有效日期" prop="youxiaoriqi">
              <el-input v-model="ruleForm.youxiaoriqi" 
                placeholder="有效日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="违规内容" prop="weiguineirong">
                <el-input
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.weiguineirong" label="违规内容" prop="weiguineirong">
                    <span>{{ruleForm.weiguineirong}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.nianling = true;
          ...
          if(o=='zhunjiachexing'){
            this.ruleForm.zhunjiachexing = obj[o];
	    this.ro.zhunjiachexing = true;
            continue;
          }
          if(o=='jiashizhengtu'){
            this.ruleForm.jiashizhengtu = obj[o];
	    this.ro.jiashizhengtu = true;
            continue;
          }
          if(o=='youxiaoriqi'){
            this.ruleForm.youxiaoriqi = obj[o];
	    this.ro.youxiaoriqi = true;
            continue;
          }
          if(o=='weiguineirong'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zhengjianxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zhengjianxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    jiashizhengtuUploadChange(fileUrls) {
	this.ruleForm.jiashizhengtu = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
